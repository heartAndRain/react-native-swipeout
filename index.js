...
const Swipeout = React.createClass({
  ...
  _calculateBtnsWidth: function(btns, defaultBtnWidth) {
    var btnsWidth = 0;
    btns.forEach((btn) => {
      if (btn.width && typeof btn.width === 'number') {
        btnsWidth += btn.width
      } else {
        btnsWidth += defaultBtnWidth || 0
      }
    })
    return btnsWidth
  },
  ...
  _handlePanResponderGrant: function(e: Object, gestureState: Object) {
    ...
    this.refs.swipeoutContent.measure((ox, oy, width, height) => {
      this.setState({
        btnWidth: (width/5),
        btnsLeftWidth: this.props.left ? this._calculateBtnsWidth(this.props.left, (width/5)) : 0,
        btnsRightWidth: this.props.right ? this._calculateBtnsWidth(this.props.right, (width/5)) : 0,
        swiping: true,
